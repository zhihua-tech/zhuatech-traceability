...
package cn.zhuatech.traceability.config;
import cn.zhuatech.traceability.domain.DomainCatalog;
import cn.zhuatech.traceability.model.*;
import cn.zhuatech.traceability.repository.*;
...
@Configuration public class DataInitializer {
 ...
 @Bean CommandLineRunner seed(BusinessRecordRepository records,SystemSettingRepository settings,DomainCatalog catalog) {
  return args -> { if(records.count()>0)return;
            ...
            settings.save(new SystemSetting("recordNoRule", "TRACE-{YYYY}-{SEQ}"));
   ...
   int sequence=1; for(var module:catalog.modules()) {
    String no="TRACE-DEMO-"+String.format("%03d",sequence);
