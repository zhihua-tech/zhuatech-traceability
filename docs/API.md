...
| POST | `/domain/decision` | 执行产品全链路追溯系统专属领域规则 |
...
| --- | --- | --- |
| `batchNo` | String | 批次编号 |
| `materialLots` | int | 投入物料批次数 |
| `linkedMaterialLots` | int | 已关联物料批次数 |
| `processSteps` | int | 应记录工序数 |
| `recordedSteps` | int | 已记录工序数 |
| `serialCoverageRate` | double | 序列号覆盖率(%) |
